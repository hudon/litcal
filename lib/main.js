...
function getCelebrations(cache, _y, _m, _d) {
  ...
  cacheWriteOne(cache, easterDate, { name: 'Easter' });

  if (date.getUTCMonth() === 11 && date.getUTCDate() === 25) {
    cacheWriteOne(cache, date, { name: 'Christmas Day' });
  }

  {
    // TODO:
    // 1. get the season, the temporal days (only compute Ordinary Time weeks)
    // 2. get the sanctal days (we just have St James for now)
    // 3. kick out what needs to get kicked out, promote, demote, etc.
  }

  const cels = cacheRead(cache, date);
...
function cacheWriteMany(cache, date, cels) {
  ...
  // cels is an Array, so we make keys out of it to prevent duplicates
  Object.assign(
    cache[ykey][m][d - 1],
    Object.fromEntries(cels.map(e => [cacheKey(e), e]))
  );
}

function cacheWriteOne(cache, date, cel) {
  cacheWriteMany(cache, date, [cel]);
}
