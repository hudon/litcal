...
function getCelebrations(cache, _y, _m, _d) {
  ...
  if (!cels.some(el => el.cycle === 'temporal')) {
    ...
    if (date <= baptismDate) { // Christmas of this litYear
      // TODO will skip Jan 1 because it's always Solemnity of Mary?
      ...
      const mkSundayName = w => w + 
        (w % 10 === 1 ? 'st' : w % 10 === 2 ? 'nd' : w % 10 === 3 ? 'rd' : 'th') +
        ' Sunday in Ordinary Time';
      const mkWeekdayName = (c, w) =>
        `${dayNames[c.getUTCDay()]} of week ${w} in Ordinary Time`;
      let cursor = createOffsetDate(baptismDate, 1);
      let week = 1;
      while (cursor < ashWedDate) {
        if (cursor.getUTCDay() === 0) {
          week++;
          cacheWrite(cache, cursor, createTemp(mkSundayName(week), 6));
        } else {
          cacheWrite(cache, cursor, createTemp(mkWeekdayName(cursor, week), 13));
        }
        cursor.setUTCDate(cursor.getUTCDate() + 1);
      }
      week = 35;
      cursor = createOffsetDate(adventStartDate, -1)
      while (cursor > pentecostDate) {
...
function cacheKey(cel) {
  return cel.name.toLowerCase().replace(/[\s0-9]/g, "");
}
