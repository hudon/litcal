...
function getCelebrations(cache, _y, _m, _d) {
  ...
  if (!cels.some(el => el.cycle === 'sanctoral')) {
    // TODO Mary mother of the Church, Immaculate Heart, Thanksgiving
    ...
    getFixedSanctorale().forEach(({ date: [m, d], name, rank }) => {
      const highRank = getHighestRank(cacheRead(cache, date).map(c => c.rank))
  ...
  if (!cels || !cels.length) {
    console.log('date cels', date, cels, cache.y2025[0])
...
function getFixedSanctorale() {
  return [
    ...
    { date: [8, 9], name: 'Saint Peter Claver, Priest', rank: 10 },
    ...
    { date: [10, 13], name: 'Saint Frances Xavier Cabrini, Virgin', rank: 10 },
    ...
    { date: [10, 24], name: 'Saint Andrew Dŭng-Lạc, Priest, and Companions, Martyrs', rank: 10 },
    ...
    { date: [11, 12], name: 'Our Lady of Guadalupe', rank: 7 },
