...
function getCelebrations(cache, _y, _m, _d) {
  ...
  // TODO make sure that when we write accidental celebrations to the cache (eg. we computed easter so we write it), we write all the celebrations for that date.
  // otherwise subsequent requests for that date
  const easterDate = computeEasterGregorian(date.getUTCFullYear());
  if (easterDate.getTime() === date.getTime()) {
    cacheWrite(cache, date, [{ name: 'Easter' }]);
  }

  if (date.getUTCMonth() === 11 && date.getUTCDate() === 25) {
    cacheWrite(cache, date, [{ name: 'Christmas' }]);
  }
