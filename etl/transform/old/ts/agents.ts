const fetchReadings = async (
	...
	for (let i = startDay; i <= numDays; i++) {
		...
		$("h3.name").each(function () {
			...
			if (
				readingTitle.length === 0 ||
				titleLower === "verse before the gospel" ||
				titleLower.startsWith("at the procession")
			) {
				return
			}
			...
			readingsData[dateKey][readingRefKey] = $(this)
				.parent()
				.next()
				.text()
				.replace(/\s+/g, " ")
...
const findInvalidLCAPIData = (data: LCAPIYear): boolean => {
	...
	for (let [lcDate, lcCelebKeys] of Object.entries(errorCelebrations)) {
		if (lcCelebKeys.length === 1) {
			logger.error(
				`Missing fallback ferial celebration for ${lcDate} for ${lcCelebKeys[0]}`,
			)
			errorsFound = true
		}
	}
...
const mergeDataForYear = (
	...
	const prevYearData: LCAPIYear = removeCruft(
		JSON.parse(fs.readFileSync(prevFileName, "utf8")),
	)
	...
	for (const [lcCelebKey, lcCelebration] of Object.entries(
		prevYearData["LitCal"],
	)) {
		const date = new Date(lcCelebration["date"] * 1000)
		if (adventDate <= date) {
